...
**棋盘结构说明：** 棋盘采用 9 列网格结构，使用一维列表 `digit_list` 存储数字。列表中的每个元素代表一个格子，0 表示空格（非数字），1-9 表示实际数字。列表按照先行后列的顺序排列，每 9 个元素构成一行。
...
**功能描述：** 判断两个位置是否满足配对条件。配对规则基于 NumberMatch 游戏的经典规则：

1. 两个数字必须相同，或两个数字之和为 10
2. 两个位置之间必须没有其他非空数字
3. 配对路径必须满足以下任一条件：
   - 相同行（水平相邻或间隔一至多格）
   - 相同列（垂直相邻或间隔一至多格）
...
# 相同行配对
print(board.is_matching(0, 8))   # True (1 和 9 和为 10)
# 跨行首尾配对
print(board.is_matching(0, 17))  # True (1 和 1 相同)
# 间隔有数字，无法配对
print(board.is_matching(0, 1))   # False (中间有 0 但间隔一个非空)
```
...
board.clear()
# clear 后 digit_list 变为 [1, 2, 3, 4, 5, 6]，空行被移除
```
...
**功能描述：** 拷贝填充。将棋盘中剩余的非空数字复制到列表末尾，实现数字的填充和重排。此方法通常在 `clear()` 操作后调用，将已消除的数字补充到棋盘底部。
...
```python
board = Board()
board.set_board([1, 2, 0, 0, 0, 0, 0, 0, 0,
                 3, 4, 0, 0, 0, 0, 0, 0, 0])
board.match(0, 1)  # 消除 1 和 2
board.clear()      # 清理后 digit_list 为 [3, 4]
board.fill()       # 填充后 digit_list 为 [3, 4, 3, 4]
```
...
**功能描述：** 执行配对消除操作。首先调用 `is_matching()` 判断两个位置是否满足配对条件，如果可以配对，则将两个位置的数字设为 0（表示消除），然后调用 `clear()` 清理空行。
...
2. **连续操作：** 配对消除后会自动清理空行，无需额外调用 `clear()`
